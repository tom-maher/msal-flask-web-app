...
Function ConfigureApplications {
    ...
    $appName = "MSAL-Python-WebApp"
    # Create the webApp AAD application
    Write-Host "Creating the AAD application ($appName)"
    # create the application 
    $webAppAadApplication = New-MgApplication -DisplayName "$appName" `
        -Web `
    @{ `
    ...
    if ($null -eq $owner) { 
        New-MgApplicationOwnerByRef -ApplicationId $currentAppObjectId  -BodyParameter @{"@odata.id" = "htps://graph.microsoft.com/v1.0/directoryObjects/{0}" -f $user.Id }
